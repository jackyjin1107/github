...
Page({
  ...
  //   if (type === 'next' || that.data.length < that.data.limitPage)
  //     wx.showToast({
  //       icon: "none",
  //       title: "最后一页了"
  //     }) else if{    
    ...
    query.find({
      success: function(res) {
        ...
        for (var i = 0; i < res.length; i++) {
          ...
          // tempGoods.packModel = res[i].packModel ;
          tempGoods.reserve = res[i].reserve;
          tempGoods.costPrice = res[i].costPrice;
          tempGoods.retailPrice = res[i].retailPrice;
          tempGoods.class_text = res[i].goodsClass;
          tempGoods.product_info = res[i].product_info;
          tempGoods.bad_num = res[i].bad_num;
          tempGoods.warning_num = res[i].warning_num;
          //JLP MODIFY
          // tempGoods.goodsName = res[i].goodsName || '';
          // tempGoods.goodsIcon = res[i].goodsIcon || '';
          // tempGoods.regNumber = res[i].regNumber || '';
